...
def sentence3() -> Expr:
    ...
    exp1 = alive1 % ((alive0 & (~killed0)) | ((~alive0) & born0))
    exp2 = ~(alive0 & born0)
    exp3 = born0
    ans = conjoin(exp1,exp2,exp3)
    return ans
    util.raiseNotDefined()
...
def entails(premise: Expr, conclusion: Expr) -> bool:
    ...
    """
    "*** BEGIN YOUR CODE HERE ***"
    logic = conjoin(premise, (~conclusion))
    dict_l = findModel(logic)
    if dict_l != False:
        return False
    return True
    util.raiseNotDefined()
...
def plTrueInverse(assignments: Dict[Expr, bool], inverse_statement: Expr) -> bool:
    ...
    """
    "*** BEGIN YOUR CODE HERE ***"
    if pl_true(inverse_statement, assignments) == False:
        return True
    return False
    util.raiseNotDefined()
    "*** END YOUR CODE HERE ***"
...
def atMostOne(literals: List[Expr]) -> Expr:
    ...
    for exp1, exp2 in combs:
        exp = (~exp1) | (~exp2)
        logic_list.append(exp)
    logic = conjoin(logic_list)
    return logic
    util.raiseNotDefined()
    "*** END YOUR CODE HERE ***"
...
def exactlyOne(literals: List[Expr]) -> Expr:
    ...
    """
    "*** BEGIN YOUR CODE HERE ***"
    exp1 = atLeastOne(literals)
    exp2 = atMostOne(literals)
    logic = exp1 & exp2
    return logic
    util.raiseNotDefined()
    "*** END YOUR CODE HERE ***"
...
def positionLogicPlan(problem) -> List:
    ...
    for t in range(50):
        ...
        pos_list = []
        for pos_x, pos_y in non_wall_coords:
            pos_list.append(PropSymbolExpr(pacman_str, pos_x, pos_y, time = t))
        ...
        for pos_x, pos_y in non_wall_coords:
            KB.append(pacmanSuccessorAxiomSingle(pos_x ,pos_y ,time = t+1, walls_grid= walls_grid))
    return None
    util.raiseNotDefined()
    "*** END YOUR CODE HERE ***"
...
def foodLogicPlan(problem) -> List:
    ...
    for t in range(50):
        ...
        food_list = []
        for f in food:
            food_list.append(~PropSymbolExpr(food_str, f[0], f[1], time = t))
        goal = conjoin(food_list)
        ...
        for pos_x, pos_y in non_wall_coords:
            KB.append(pacmanSuccessorAxiomSingle(pos_x ,pos_y ,time = t+1, walls_grid= walls))
        
        for f in food:
            exp1 = ~PropSymbolExpr(food_str, f[0], f[1], time = t+1)
            exp2 = (PropSymbolExpr(pacman_str, f[0], f[1], time = t) & PropSymbolExpr(food_str, f[0], f[1], time = t))
            KB.append(exp1 % exp2 | ~PropSymbolExpr(food_str, f[0], f[1],time = t))
    return None
    util.raiseNotDefined()
    "*** END YOUR CODE HERE ***"
...
def slam(problem, agent) -> Generator:
    ...
    for t in range(agent.num_timesteps):
        ...
        percepts = agent.getPercepts()
        percepts_rule = numAdjWallsPerceptRules(t, percepts)
        KB.append(percepts_rule)
        ...
        possible_locations = []
        for pos in non_outer_wall_coords:
            proved_not_in = findModel(conjoin(KB) & ~PropSymbolExpr(pacman_str, pos[0], pos[1], time = t))
            proved_in = findModel(conjoin(KB) & PropSymbolExpr(pacman_str, pos[0], pos[1], time = t))
            if proved_not_in == False:
                KB.append(PropSymbolExpr(pacman_str, pos[0], pos[1], time = t))
            elif proved_in == False:
                KB.append(~PropSymbolExpr(pacman_str, pos[0], pos[1], time = t))
            if proved_in:
                possible_locations.append((pos[0], pos[1]))
